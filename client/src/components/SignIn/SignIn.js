...
const SignIn = () => {
  ...
  const formSubmitHandler = (event) => {
    ...
    if(countofErrors == 0){
      axios.post("http://localhost:3709/signin", {
        email: values.email,
      ...
      .then((response) => {
          ...
          if (response.data.token) {
              ...
              // Agora faz a navegação
              history.push("/home");
          } else {
              setLogingWarning("Erro ao fazer login. Tente novamente.");
          }
      })
      .catch((error) => {
          ...
          if (error.response && error.response.data.message) {
              ...
              setLogingWarning("Erro de conexão. Tente novamente.");
          }
      })
